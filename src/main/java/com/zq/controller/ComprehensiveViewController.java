	...
	*/
    ...
    public String capexPhaseStatus(HttpServletRequest request) throws Exception {		
		...
		for(BasCAPEXProject capexProject:capexList){
			...
			if(dataUpdateDate != null && capexProject.getModifyTime() != null && dataUpdateDate.before(capexProject.getModifyTime())){
				dataUpdateDate = capexProject.getModifyTime();
			}
...
//			System.out.println("property=="+property);
			BasCAPEXInvestPlan plan =iBasCAPEXInvestPlanService.getBasCAPEXInvestPlanByYearAndBasCAPEXProject(year,capexProject.getProjCode());
			String shuxing="";
			if(plan != null){
		...
		List<BasFirstBill> firstBillList=projectIDSet.size()>0?iBasFirstBillService.getFirstBillByYear(year):java.util.Collections.EMPTY_LIST;
		double contractValue=0d;
		for(BasFirstBill firstBill:firstBillList){
			if(dataUpdateDate == null){
				dataUpdateDate = firstBill.getLastUpdateTime();
			}
			if(dataUpdateDate != null && firstBill.getLastUpdateTime() != null && dataUpdateDate.before(firstBill.getLastUpdateTime())){
				dataUpdateDate = firstBill.getLastUpdateTime();
			}
		...
		//ps.clear();
		//ps.put("capexProjectCodeArray", projectCodeSet);
		List<BasSecondBill	> secondBillList=projectIDSet.size()>0?iBasSecondBillService.getSecondBillByYear(year):java.util.Collections.EMPTY_LIST;;
		for(BasSecondBill secondBill:secondBillList){
			if(dataUpdateDate == null){
				dataUpdateDate = secondBill.getModifyTime();
			}
			if(dataUpdateDate != null && secondBill.getModifyTime() != null && dataUpdateDate.before(secondBill.getModifyTime())){
				dataUpdateDate = secondBill.getModifyTime();
			}
