...
<div style="position: relative;">
	<div class="sumValue"
		style="color:<%=(totalActualValue>totalPlanValue)?"#BF100F":"#0ACC4B" %>">
		<b><%=TypeUtils.formatWanMoney(totalActualValue) %></b>&nbsp;
	</div>
	<div
		style='width: 90%; margin: 3px auto; height: 8px; background-color: #E8E9ED;'>
		<%Double percent=totalActualValue/totalPlanValue*100; %>
		<div
			style=' height:100%;width:<%=totalPlanValue!=0?percent.intValue()>100?100:percent.intValue():0%>%;background-color:<%=(totalActualValue>totalPlanValue)?"#BF100F":"#0ACC4B" %>;''></div>
	</div>
	...
	<div
		style='width: 90%; margin: 2px auto; height: 1px; background-color: #E8E9ED;'></div>
	...
	<div class="helpFloatDIV" id="showSummaryShouRuTipDIV"
		...
		<div class="showHelpeContent"
			style="line-height: 23px; max-width: 700px;">
			<div>
				本年分公司真实收入：取自分公司财务报表的收入，包括以下四类。<br>
				...
				产品合作收入：省公司签约销售政企自有产品，各省按照结算方案采取收入分成或成本结算模式结算给政企（不同产品具体结算方案不一样）
			</div>
		</div>
	</div>
</div>
