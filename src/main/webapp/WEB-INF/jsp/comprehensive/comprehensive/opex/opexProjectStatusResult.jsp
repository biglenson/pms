...
<table class="statusTable" style="margin:0 auto;" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td >
			<div style="padding-bottom:20px;"><img src="<%=path %>/images/cmcc/liucheng2.png"></div>
		</td>
	</tr>
	...
	<tr>
		...
		<td >
			<div class="cmcc_phase_kuang" style="background-position: 15px  0px">
				<div class="cmcc_phase_valueDIV linkURL" style="width:119px;" onclick="openOpexProjectList(-1);" ><%=TypeUtils.formatCount(totalCount) %></div>
				<%for(int i=0,j=phaseName.length;i<j;i++){%>
				<div class="cmcc_phase_valueDIV linkURL"  style="width:119px;" onclick="openOpexProjectList(<%=i %>);"  ><%=TypeUtils.formatCount(allCount[i]) %></div>
				<%} %>
			</div>
	</tr>
	<tr><td style="height:10px;"></td></tr>
	<tr>
		...
		<div style="width:<%=2+119*8 %>px;">
			...
			<%for(int i=0,j=phaseName.length;i<j;i++){%>
			<div class='cmcc_phase_moenyTip' style="width:119px;"><%=TypeUtils.xmlEncoderForIE(moneyNames[i]) %>&nbsp;</div>
			<%} %>
		</div>
		</td>
	</tr>
	<tr>
		...
		<div style="width:<%=2+119*8 %>px;">
			<div class="cmcc_phase_moneyValue" style='width:119px;'><%=TypeUtils.formatWanMoney(totalMoney) %>&nbsp;</div>
			<%for(int i=0,j=phaseName.length;i<j;i++){%>
				<%if(i!=0){
					if(i==3){ %>
				...
				<%}else{ %>
					<div class="cmcc_phase_moneyValue" style='width:119px;' ><%=TypeUtils.formatWanMoney(allMoney[i]) %>&nbsp;</div>
				<%}}else{ %>
