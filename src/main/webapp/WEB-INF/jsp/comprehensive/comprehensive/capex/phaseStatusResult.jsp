	...
	<table class="statusTable" style="margin: 0 auto;" border="0"
			...
			<td>
				<div class="cmcc_phase_kuang" style="width:<%=2+103*8 %>px;">
					<div class="cmcc_phase_valueDIV linkURL"
						onclick="openCapexProjectList(-1,0);"><%=TypeUtils.formatCount(totalCount) %></div>
					<%for(int i=0,j=phaseName.length;i<j;i++){%>
					<div class="cmcc_phase_valueDIV linkURL"
						onclick="openCapexProjectList(<%=i %>,0);"><%=TypeUtils.formatCount(allCount[i]) %></div>
					<%} %>
		...
		<tr>
			...
			<td>
				<div class="cmcc_phase_kuang" style="width:<%=2+103*8 %>px;">
					<div class="cmcc_phase_valueDIV linkURL"
						onclick="openCapexProjectList(-1,1);"><%=TypeUtils.formatCount(newTotalCount) %></div>
					<%for(int i=0,j=phaseName.length;i<j;i++){%>
					<div class="cmcc_phase_valueDIV linkURL"
						onclick="openCapexProjectList(<%=i %>,1);"><%=TypeUtils.formatCount(newCount[i]) %></div>
					<%} %>
					<div style="clear: both;"></div>
			</td>
		</tr>
		...
		<tr>
			...
			<td><div style="width:<%=2+103*8 %>px;">
				...
				%>
					<div class="cmcc_phase_moneyValue"><%=TypeUtils.formatWanMoney(v, "")%>&nbsp;
					</div>
					<%} %>
					<div style="clear: both;"></div>
				</div></td>
