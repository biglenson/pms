	...
	<div style="width:190px;float:left;margin-left: 10px;">
		<div style="margin-top:8px;">
			...
			<div style='width:95%;margin:10px auto;height:8px;background-color:#E8E9ED;'>
				<div style=' height:100%;width:<%=opexValue!=0? (opexShiYongValue>opexValue?100:opexShiYongValue/opexValue*100):0%>%;background-color:#0ACC4B'></div>
			</div>
			<div style="padding-bottom:18px;border-bottom:1px solid #EBE8EF;">预算总额&nbsp;: <b style="font-size: 14px;"><%=TypeUtils.formatWanMoney(opexValue,"")  %></b></div>
		</div>
		
		
		<div style="margin-top:15px;">
			<div style="padding-bottom:10px;">预算池使用&nbsp;:</div>
			<div style="padding-bottom:10px;font-size: 16px;font-weight: bold;"><%=TypeUtils.formatWanMoney(poolShiYongValue,"") %></div>
			<div style='width:95%;margin:10px auto;height:8px;background-color:#E8E9ED;'>
				<div style=' height:100%;width:<%=poolValue!=0?(poolShiYongValue>poolValue?100: poolShiYongValue/poolValue*100):0%>%;background-color:#0ACC4B'></div>
			</div>
			<div style="padding-bottom:8px;">动态预算池总额&nbsp;: <b style="font-size: 14px;"><%=TypeUtils.formatWanMoney(poolValue,"")  %></b></div>
		</div>
	</div>
	<div id="touziDetialOpexDIV" style="margin-left:220px;">
		<div style="height:250px;">
			<table id="touziDetialOpexDIVTable" class="listTable"  cellpadding="0" cellspacing="0" border="0">
			<thead>
				<tr>
			...
			<tbody>
				...
				%>
				<tr>
					<td align="center"><div class="linkURL" onclick="openDepartmentView(<%=role.getId()%>)"><%=TypeUtils.xmlEncoderForIE(role.getName()) %></div></td>
