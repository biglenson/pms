...
<form name="frm" action="<%=path%>/datamap/getTaskDone" method="POST">
...
<%=UIUtils.tabPanelEnd(request)%>
	...
	<ul id="tab" style="margin:0;padding:0;list-style:none;overflow:hidden;border-top:1px solid #DDD;background:#F4F4F4;">
        <li style="float:left;width:60px;height:30px;background:#F4F4F4;border:0px;text-align:center;line-height:30px;cursor:pointer;border-left: 1px solid #DDD; ">待办</li>
        <li style="float:left;width:60px;height:30px;background:#FFF;border:0px;text-align:center;line-height:30px;cursor:pointer;">已办</li>
    </ul>
				...
				for (TaskTodoItemVO taskDone : taskDoneList) {
					if (taskDone.getEvalFor() == isProject){
						...
						<tr class="listTableTR">
							...
							<td><fmt:formatDate value="<%=taskDone.getCreateTime()%>" pattern="yyyy-MM-dd HH:mm:ss"/></td>
						</tr>
						<%
					}
				} 
