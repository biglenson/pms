...
EasyTrack.DataTable.prototype.initialTable=function(){
	...
	//如果是自动增长高度，则忽略外层的container 的高度
	var table=self.table;
	ET.Utils.addClass(table,'dataTable');
...
/**
 * 增加的一个方法，处理 简单表格的表头跟内容宽度不一致的问题，
 * 原理是将 表头的第一行td 跟内容的第一行td 宽度对齐
 * 只适合 没有合并单元格的表格
 */
EasyTrack.DataTable.prototype.need_alignTableHead=function(){
	
	var _self=this;
	var table=_self.table;
	var tTableHead1=_self.body_theadTable;
	var tTableHead2=_self.column_theadTable;
	
	var alignHead=function(table1, table2){
		var row1=table1.rows[0];
		var row2=table2.tBodies[0]&&table2.tBodies[0].rows[0]? table2.tBodies[0].rows[0]:null;
		if(row2!=null){
			var td1s=row1.cells;
			var td2s=row2.cells;
			if(td1s.length!=td2s.length){
				return;
			}
			for(var i=0,j=td1s.length;i<j;i++){
				var td1=td1s[i];
				var td2=td2s[i];
				if(td1!=null&&td2!=null){
					var td1Width=td1.offsetWidth;
					var td2Width=td2.offsetWidth;
					var width=td1Width-td2Width;
					if(width<0){
						width=0-width;
					}
					if(width>2){
						var th1DIV=td1.children[0];
						if(th1DIV&&th1DIV.tagName.toLowerCase()=='div'){
							th1DIV.style.width=td2Width-1+"px";
						}
					}
				}
			}
	...
	alignHead(tTableHead1,table);
	if(tTableHead2!=null){
		alignHead(tTableHead2,table);
	}
};
//将表格的表头对齐
EasyTrack.DataTable.prototype.alignTableHead=function(){
    var self=this;
    var alignTableHeadFlag=self.config.need_alignTableHead==true;
    if(alignTableHeadFlag){
    	self.need_alignTableHead();
    }
	var container=self.container;
	var body_theadTable=self.body_theadTable;
	var body_theadContainer=self.body_theadContainer;
...
EasyTrack.DataTable.prototype.loadLazyRow=function(loadRows){
	...
	if(fixed>0){
		...
		self.column_tbody.scrollTop=self.body_tbody.scrollTop;
...
EasyTrack.FixedTwoTable.prototype.initTable=function(){
	...
	if(seperateDIV==null){
		...
		ET.Utils.addClass(seperateDIV,"DataTableFixedTable__seperateDIV");
		//这里写死了，以后可能需要放开一些东西。。。。
	...
	cache['seperateDIVOffsetWidth']=seperateDIV.offsetWidth;
	...
	var fixeTableHead=function (table,position,tableDIV){
			...
			if(position=='left'){
				...
				if(self.config.leftTableWidth==undefined && typeof self.config.leftTableWidth !="function" ){
...
EasyTrack.FixedTwoTable.prototype.bindEvent=function(){
	//宽度重新计算
		...
		EventUtil.addHandler(self.rightTableWrapDIV,'scroll',function(event){
			...
			self.leftTableWrapDIV.scrollTop=scrollTop;
			self.rightTableHeadDIV.scrollLeft=self.rightTableWrapDIV.scrollLeft;
			if(isRightFun){
		...
		EventUtil.addHandler(self.leftTableWrapDIV,'scroll',function(){
			self.leftTableHeadDIV.scrollLeft=self.leftTableWrapDIV.scrollLeft;
			self.rightTableHeadDIV.scrollTop=self.leftTableWrapDIV.scrollTop;
			if(isLeftFun){
				leftTableScrollEvent.call(self,self.leftTableWrapDIV);
			}
		});
		
		var doScrollData=function(){
			if(self.config.lazyLoad){
		...
		var doMouseWheel=function(event){
			event=event||window.event;
...
//			EasyTrack.log('old scrollTop='+scrollTop);
		...
		var doMouseWheel2=function(event){
			event=event||window.event;
			var delta=EventUtil.getWheelDelta(event);
			//EasyTrack.log('delta='+delta);
			var scrollTop=self.leftTableWrapDIV.scrollTop;
			//EasyTrack.log('old scrollTop='+scrollTop);
			scrollTop-=(delta/120)*80;
			self.leftTableWrapDIV.scrollTop=scrollTop;
			self.rightTableWrapDIV.scrollTop=scrollTop;
		...
		EventUtil.addHandler(self.leftTableWrapDIV,EasyTrack.browser.firefox?'DOMMouseScroll':'mousewheel',doMouseWheel2);
		EventUtil.addHandler(self.rightTableWrapDIV,EasyTrack.browser.firefox?'DOMMouseScroll':'mousewheel',doMouseWheel);
	...
	ET.Utils.addEvent(document,"keydown",function(event){
		event=event||window.event;
...
EasyTrack.FixedTwoTable.prototype.resize=function(caculateHeightAgain){
	...
	self.leftTableWrapDIV.scrollTop=self.rightTableWrapDIV.scrollTop;
	self.rightTableHeadDIV.scrollLeft=self.rightTableWrapDIV.scrollLeft;
	self.leftTableHeadDIV.scrollLeft=self.leftTableWrapDIV.scrollLeft;
};
EasyTrack.FixedTwoTable.prototype.caculateDIVWidth=function(){
	var self=this;
	var _width=self.config.leftTableWidth;
	if( typeof self.config.leftTableWidth =='function'){
		_width=self.config.leftTableWidth();
	}
	var leftDIVOffsetWidth=self.config.leftTableWidth!=null?_width:self.cache['leftTableOffsetWidth'];
...
EasyTrack.Export=function(config){
	/**
	 * headLinkArray ,指定导出的生成页面需要引入的style 文件，非必要不要自己配置，默认会引入页面自己的style 文件
	 * exportDom 指定需要导出dom 默认document.body
	 * exportHtml 默认是exportDom.outerHTML ，也可以自己指定导出的内容
	 */
	
	var headLinkArray=[];
	if(config.headLinkArray!=null&& typeof config.headLinkArray=='function'){
		headLinkArray=config.headLinkArray();
	}else if( config.headLinkArray!=null&& typeof config.headLinkArray=='array'){
		headLinkArray=config.headLinkArray;
	} else{
		//页面的 style
		...
		for(var i=0,j=linkList.length;i<j;i++){
			...
			if(link.rel=="stylesheet"||link.type=='text/css'){
				headLinkArray.push(link.outerHTML);
			}
		}
	}
	/**
	...
	for(var i=0,j=styles.length;i<j;i++){
		styleArray.push(styles[i].innerHTML);
	}
	/**
	 * phantomjs  cookie 配置
	 */
	var cookie={
			JSESSIONID:ET.Utils.getCookie('JSESSIONID')
	}
	...
	html.push("<html>");
		...
		html.push("<title>");
			//导出页面的title
			html.push(document.title||config.title);
			html.push("</title>");	
			html.push("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"> ");
			
			for(var i=0,j=headLinkArray.length;i<j;i++ ){
				html.push(headLinkArray[i]);
			}
			...
			html.push("</style>");
			html.push("</head>");
			html.push("<body style='height:auto;overflow:auto'>");
			var exportDom=config.exportDom;//需要导出的Dom 
			if(exportDom==null){
				exportDom=document.body;
			}
			if(exportDom!=document.body){
				//去除导出DOM 中间的script 标签（这些标签不应该再执行）
				var scripts=exportDom.getElementsByTagName('script');
				for(var i=scripts.length-1;i>=0;i--){
					scripts[i].parentNode.removeChild(scripts[i]);
				}
			}
			var jQuery=window.jQuery;
			var isIE78=ET.is_ie7||ET.is_ie8;
			if(isIE78&&jQuery!=undefined){
			}
		
			var exportHtml=null;
			if(config.exportHtml==undefined){
				//除body 为innerHTML,其他 都是 outerHTML,
				exportHtml=exportDom!=document.body?exportDom.outerHTML:exportDom.innerHTML;
			}else{ 
				//指定导出内容
				exportHtml=config.exportHtml;
			}
			html.push(exportHtml);
			html.push("</body>");
	html.push("</html>");
	// 开始 将页面内容提交到后台
	var frm=document.getElementById("export_dom_frm");
	if(frm!=null){
		var children=frm.children;
		while(children.length>0){
			frm.removeChild(children[0]);
		}
	}else{
	...
	if(!config.action){
		...
		config.action=contextPath+"/ExportExcelAction.do?operation=exportDom";
	}
	frm.action=config.action;
	...
	if(parameter!=null && typeof parameter=='object'){
		for(key in parameter){
			createHidden(key, parameter[key]);
		}
	}
	
	createHidden('exportHtml',html.join("\n"));
	//浏览器窗口大小
	var viewportSize =config.viewportSize ;
	if(viewportSize==undefined){
		viewportSize={};
		var exportDom=config.exportDom;
		if(exportDom==null){
			exportDom=document.body;
		}
		var height=Math.max(exportDom.scrollHeight,exportDom.offsetHeight) ;
		var width=Math.max(exportDom.scrollWidth,exportDom.offsetWidth);
		viewportSize.width=width+"px";
		viewportSize.height=(height+20)+"px";
	}
	createHidden('viewportSize',JSON.stringify(viewportSize));
	var paperSize  =config.paperSize  ;
	if(paperSize!=null){
		createHidden('paperSize',JSON.stringify(paperSize));
	}
	createHidden('cookieObj',JSON.stringify(cookie));
	frm.submit();
};
